...
1. Download **Magwell.dmg** from [Releases](https://github.com/quillerpl/magwell/releases/latest).
2. Open it and drag **Magwell** to **Applications**.
3. Open Applications and **right-click Magwell → Open**, then click **Open** in the dialog.

> **Why right-click the first time?** Magwell isn't signed with a paid Apple developer
...
Magwell then walks you through the one permission it needs.

### The permission

macOS requires **Accessibility** access before *any* app can paste on your behalf — pasting
means pressing ⌘V for you, and macOS treats that as controlling your computer. Magwell asks
...
- **Nothing is written to disk.** History lives in memory. Quitting Magwell or restarting your
  Mac clears it completely — the same way Windows' clipboard history behaves.
- **Nothing is sent anywhere.** Magwell has no network code beyond checking for its own updates.
...
```bash
git clone https://github.com/quillerpl/magwell.git
cd magwell
...
```
Sources/MagwellKit/       everything: capture, paste, hotkeys, panels
...
Sources/Magwell/          thin executable shell
Tests/MagwellKitTests/    capture rules, store, search, layout maths
```
...
- `make-signing-cert.sh` creates a self-signed `Magwell Dev` certificate (trusted **for code
...
```bash
/Applications/Magwell.app/Contents/MacOS/Magwell --check-trust
```

If it prints `false` while the switch is on, remove Magwell from the Accessibility list with
**–**, then re-add it with **+**.
...
```bash
/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister -f /Applications/Magwell.app
```
...
**⌘⇧V is taken over globally.** Several apps use it for "Paste and Match Style"; Magwell's
version does the same thing, and adds it to apps that lack it. To change it, edit
`registerHotKeys()` in [AppDelegate.swift](Sources/MagwellKit/AppDelegate.swift).
